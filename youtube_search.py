...
class YouTubeSearch:
    def __init__(self, search_terms: dict):
        ...
        self.video = None
        self.cached_songs = [re.search(r"_(.*).mp3", x).groups()[0] for x in glob("media/*mp3")]
    ...
    def _search(self) -> Dict[str, str]:
        ...
        request = youtube.search().list(part="snippet", q=self.search_terms, type="video", maxResults=1,)
    ...
    def _download_youtube_video(self) -> None:
        mp3_output = f"media/{self.video['id']}_{self.search_terms_str.replace(' ', '')}.mp3"
        ...
        try:
            ...
            os.system(f'ffmpeg -y -i {self.video["id"]}.mp4 -f mp3 -ab 192000 -vn {mp3_output}')
